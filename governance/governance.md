...
Think of this document as the operating agreement for how PhilaCon Valley makes decisions, distributes responsibility, and grows its leadership. It's not a constitution — it's a living record of how we work today and a framework for how we'll adapt tomorrow.
...
Not all decisions are the same, and they shouldn't all be made the same way. PhilaCon Valley uses a layered approach — the weight of the process matches the weight of the decision.
...
PhilaCon Valley is not a fixed thing. The community will change as it grows, and the governance structure should change with it. What works for a community of 50 won't work for a community of 500. We'd rather plan for that now — while things are small and relationships are strong — than scramble to figure it out under pressure.
...
The mission. The values. The commitment to centering Black, Brown, LGBTQIA+, and underrepresented technologists. Governance structures can evolve, but the identity of this community is not negotiable. Any structural change that would dilute that identity is a structural change we won't make.

---

## A Note on Power

We are aware that governance structures can concentrate power in ways that contradict community values. A small team making decisions for a larger group is efficient, but it's also a power dynamic — and pretending otherwise would be dishonest.

We commit to keeping this structure accountable: open about who makes decisions, transparent about why, willing to hear criticism, and ready to change when the community needs something different. If the way we govern ever starts to feel closed, self-serving, or unresponsive — say so. That feedback is not a threat to the community. Ignoring it would be.

---

## How Community Members Can Contribute

Anyone can contribute to the handbook and to how the community operates. We welcome suggested edits or additions to existing documents, proposals for new documents or sections, and feedback on how things are working — or not working.
...
Documents in this handbook are marked with one of the following statuses:

**Draft** — Actively open for input. These are working documents and we expect them to change. Most documents start here.

**Active** — Reflects current community practice. Still open to revision through the contribution process, but changes should be well-considered and discussed before being adopted.

Major changes to the handbook — especially to the mission, values, or governance structure — will be discussed with the broader community before being finalized, following the decision-making process described above.
